...
# 4. PWM fan control on PA3 - TB9 
fan = ["fan", "PA3", "PWM", "fanF", 1]
fanVal = ["range", 5000, 300000, "speed"] 
  
fanOnOff = ["fanOnOff", "PA4", "digital", "fanOnOffF", 1] 
fanOnOffValues = ["discrete", 2, "low", "high"] 
  
lightOnOff= ["lightOnOff", "PA0", "digital", "lightOnOffF", 1]
...
def tempMeasFunc(): 
    ...
    if (value > 100):
        celPy.AdjustLocalControlPoint("fan", 300000)
    if (value < 0):
        celPy.AdjustLocalControlPoint("fan", 5000)  
...
celPy.ControlPoints = [greenLed, redLed, buzzer, fan, fanOnOff, lightOnOff] 
celPy.ControlValues = [greenValues, redValues, buzzerVal, fanVal, fanOnOffValues, lightOnOffValues] 
  
def main(): 
    prevButtonValue = 0 
    # Init temp/humidity sensor 
    celPy.AdjustLocalControlPoint("buzzer", 0)
    # Set fan to loweset speed
    celPy.AdjustLocalControlPoint("fan", 0)
    celPy.AdjustLocalControlPoint("fanOnOff", 0)
    celPy.AdjustLocalControlPoint("lightOnOff", 0)
